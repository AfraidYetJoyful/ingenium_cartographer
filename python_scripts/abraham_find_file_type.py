...
#MS The "os" library allows for interaction with the operating system (ie. moving/renaming files).
...
#MS Fuction that inputs "content" and "file_path" and writes the content to a .txt file at the given path, (overwriting if it already exists).
def exportFile(content, file_path):
    with open(file_path, 'w', encoding='utf-8') as file:
        file.write(content)
        
        
#MS Core function for searching files. "root_folder" is the starting directory where the search will start. "max_depth=4" is a parameter that sets the depth of the search to 4 sub_categories (aka 4 levels deep). 'extension=".py"' specifies that by default, the file we are looking for is in python (.py).
def find_files(root_folder, max_depth=4, extension = ".py"):
    ...
    def search_folder(folder, depth):
        ...
        try:
            for entry in os.scandir(folder): #MS Iterates over files and subdirectories in a given folder.
                ...
                elif entry.is_dir(): #MS Looks to see if "entry" is a directory. If it is, then continue recursively until max depth is reached
        ...
        try: #MS Handles errors if .zip file is not valid.
            with zipfile.ZipFile(zip_path, 'r') as z: #MS Opens .zip in read mode ('r') and closes it down afterwards.
                for file in z.namelist(): #MS Goes through all the files and directories in ZIP file.
                    if file.endswith(extension): #If the file ends with the current extension variable, then the file is added to global "output".
